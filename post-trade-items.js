...
const PAGE_TITLE = "Module:TradeItems/data";
const FILE_PATH = path.join(__dirname, "luatables", "trade-items.lua"); // Path to your local file
const VERSION_NAME = require("./gameVersion.json").gameVersion; // Replace with the version name for your edit summary

async function updateTradeItems() {
    try {
        const client = new mw({
            protocol: "https",
...
module.exports = updateTradeItems;

// Execute only if the script is run directly
if (require.main === module) {
    (async () => {
        try {
            await updateTradeItems();
